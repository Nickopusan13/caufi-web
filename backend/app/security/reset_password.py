...
async def save_token_forgot_password(
    db: AsyncSession, email: str, token: str, expire_minutes: int
):
    ...
    await db.execute(
        delete(UserPasswordResetToken).where(UserPasswordResetToken.user_id == user.id)
    )
    expire_at = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(
        minutes=expire_minutes
    )
    stmt = insert(UserPasswordResetToken).values(
        user_id=user.id, token=hashed_token, expires_at=expire_at
    )
...
async def get_user_reset_passsword(db: AsyncSession, token: str):
    ...
    query = (
        select(User)
        .join(User.password_reset_token)
        .where(UserPasswordResetToken.token == hashed_token)
    )
...
async def update_password(db: AsyncSession, id: int, password: str):
    ...
    query_token = delete(UserPasswordResetToken).where(
        UserPasswordResetToken.user_id == id
    )
